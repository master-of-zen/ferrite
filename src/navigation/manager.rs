use super::error::{NavigationError, Result};
...
impl NavigationManager {
    ...
    /// Remove a deleted file from the directory listing and navigate to the
    /// next available image Returns the path of the next image to display,
    /// or None if no images remain
    #[instrument(skip(self, deleted_path), fields(deleted_path = ?deleted_path))]
    pub fn remove_deleted_file(
        &mut self,
